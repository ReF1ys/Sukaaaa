...
export default function DashboardPage() {
  ...
  // Данные о браке
  const defectRateData: Record<PeriodType, ChartDataByPeriod> = {
    week: {
      labels: ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'],
      datasets: [
        {
  ...
  const equipmentLoadData: Record<PeriodType, ChartDataByPeriod> = {
    week: {
      labels: ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'],
      datasets: [
        {
      ...
      chart.data.datasets.forEach((dataset, i) => {
        ...
        if (!meta.hidden) {
          meta.data.forEach((element, index) => {
            const position = (element as unknown as { getCenterPoint(): { x: number; y: number } }).getCenterPoint();
            const value = dataset.data[index];
            
            // Only draw on the first dataset to avoid duplicates
            if (i === 0) {
              ...
              <div className={styles.statsRow}>
                <div className={styles.stat}>
                  <div className={styles.titlestat}>
                    <Image src="/images/all.svg" alt="machine" width={24} height={24} />
                    <h3>Всего станков</h3>
                  </div>
